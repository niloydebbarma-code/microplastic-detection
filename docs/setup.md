# Setup Guide for Microplastic Detection System
...
A web dashboard will start at `http://localhost:5000` (or your custom `--port`) showing:
- **Live camera feed** from ESP32-CAM
- **Green boxes** around detected microplastics
- **Frame and total particle counts**
- **Inference time and FPS**
- **Water quality indicator** and event log
...
```bash
python 03_esp32_integration.py --esp32 192.168.4.1 --port 5001
```

**Use a specific model file:**
```bash
python 03_esp32_integration.py --esp32 192.168.4.1 --model yolov8_microplastic_trained.pt
```
...
- **Board:** AI-Thinker ESP32-CAM with OV2640 camera sensor
...
1. **Resolution:** The camera control panel starts at SVGA (800x600). For best model compatibility and faster real-time inference, set it to VGA (640x480).
   - HD (1280x720) is available for more detail if needed.
   - SVGA (800x600) and FHD (1920x1080) are also supported, but VGA is recommended for real-time detection.
...
4. Use the **Snapshot** button in the dashboard to save annotated frames for review
...
1. **Collect Data:** Save snapshots from the dashboard and keep timestamped runs for comparison
...
- Look for: "ESP32-CAM with OV2640" or "AI-Thinker ESP32-CAM"
...
python 03_esp32_integration.py --esp32 192.168.4.1 --conf 0.35 --port 5001 --model yolov8_microplastic_trained.pt
```
...
A: This code is specifically for AI-Thinker ESP32-CAM with OV2640. Other models need pin changes.
